...
console.log({ session })
  ...
  <label className="btn m-1 flex items-center mt-0 bg-black space-x-3 opacity-90 hover:opacity-80 cursor-pointer rounded-full p-1 pr-2">          <img 
            className="rounded-full w-8 h-8" 
            src={session?.user.image} 
            alt="" 
          />
          <h2 className="text-gray-400">{session?.user.name}</h2>
          <ChevronDownIcon className="h-5 w-5" /></label>
